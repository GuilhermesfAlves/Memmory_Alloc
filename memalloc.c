...
/*
    long long* greater_unused_brk = original_brk;
    ...
    if ((greater_unused_brk == original_brk) && (*greater_unused_brk == 1)){ // não há nenhum bloco desalocado que caiba
        greater_unused_brk = dynamic_brk;
        dynamic_brk += bytes + OVERHEAD; //avança o brk
    ...
    *greater_unused_brk = 1;
    ...
    if (*(greater_unused_brk + 1) > bytes + OVERHEAD ){
        return greater_unused_brk + OVERHEAD;
    }
...
/*
if (dynamic_brk > original_brk)
    if (*greater_unused_brk != 0)
        greater_unused_brk = dynamic_brk

if (greater_unused_brk == dynamic){
    aloca mais
}
*/